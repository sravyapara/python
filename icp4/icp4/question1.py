class Employee:
    ...
    employeeCount = 0
    # Assigned deafult constructore and passed an argument empdictionary
    def __init__(self, emdetails):
        self.emp_dictionary = emdetails
        # Takes the employee count based on number of elements in the dictionary
        self.employeeCount = len(emdetails)
    ...
    def display_employee_details(self):
        ...
        for count in range(len(self.emp_dictionary)):
            if count <= len(self.emp_dictionary):
                # Prints which employee is displaying
                ...
                # Prints average salaries
        print("Employees Average Salary:", employeeSalary/self.employeeCount)

# Inherited FullTimeEmployee from Employee class
class FullTimeEmployee(Employee):
    def __init___(self, emdetails):
        Employee._init_(self, emdetails)
...
while validation[0] == 'y':
    ...
    if employeeName not in emdetails:
        emdetails[empId] = [employeeName, employeeFamily, employeeSalary, employeeDepartment]
        empId += 1
    else:
